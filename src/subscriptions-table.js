...
class SubscriptionsTable {
  ...
  getButtons() {
    const elements = this.data.map((sub) => {
      return (`
        <td>
          <button class="js-subscription-select btn btn-success" style="margin-top: 15px" data-id="${sub.id}">
    ...
    return (`
      <tr class="subs-buttons">
        ...
        ${elements.join('')}
        <td>
          <button id="open-payform" style="margin-top: 15px" class="btn btn-success">
            ${generateHtml('Обрати', 'Choose')}
          </button>
        </td>
      </tr>
